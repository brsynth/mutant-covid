...
# ============================================================
# Existing selection workbooks = source of truth
# ============================================================
aic_workbook_paths <- list(
  MS = file.path("time_series_selected", "bam_model_selection", "BAM_MS_all_strains_model_comparison.xlsx"),
  NP = file.path("time_series_selected", "bam_model_selection", "BAM_NP_all_strains_model_comparison.xlsx"),
  NS = file.path("time_series_selected", "bam_model_selection", "BAM_NS_all_strains_model_comparison.xlsx"),
  MN = file.path("time_series_selected", "bam_model_selection", "BAM_MN_all_strains_model_comparison.xlsx"),
  three_groups = file.path("time_series_selected", "bam_model_selection", "BAM_three_groups_all_strains_model_comparison.xlsx")
)
...
apply_workbook_level_overall_correction <- function(overall_global_test,
  ...
  overall_tbl <- overall_global_test %>%
    dplyr::mutate(
      ...
      overall_interpretation_adjusted = dplyr::case_when(
        overall_passes ~ "Overall test passes workbook-level multiplicity correction across all files",
        TRUE ~ "Overall test does not pass workbook-level multiplicity correction across all files"
      )
    )
  ...
  run_info_tbl <- run_info %>%
    ...
    dplyr::left_join(
      overall_tbl %>%
        dplyr::select(
          file_name, analysis_mode, selected_model,
          overall_p_raw, overall_p_adjusted,
          overall_correction_used, overall_passes,
          overall_interpretation_adjusted
        ) %>%
        dplyr::distinct(),
      by = c("file_name", "analysis_mode", "selected_model")
    )
...
apply_final_significance_logic <- function(pairwise_differences,
  ...
  pairwise_flags <- pairwise_global_tests %>%
    dplyr::select(
      file_name, analysis_mode, selected_model, prediction_type,
  ...
  if (nrow(pairwise_global_tests_mgcv) > 0) {
    mgcv_flags <- pairwise_global_tests_mgcv %>%
      ...
      dplyr::select(
        file_name, analysis_mode, comparison,
    ...
    mgcv_flags <- pairwise_differences %>%
      dplyr::distinct(file_name, analysis_mode, comparison) %>%
      dplyr::mutate(
  ...
  pairwise_differences %>%
    ...
    dplyr::left_join(
      pairwise_flags,
      by = c("file_name", "analysis_mode", "selected_model", "prediction_type", "comparison")
    ) %>%
    dplyr::left_join(
      mgcv_flags,
      by = c("file_name", "analysis_mode", "comparison")
    ) %>%
    dplyr::mutate(
...
# ============================================================
load_precomputed_selection <- function(aic_workbook_paths) {
  out <- lapply(names(aic_workbook_paths), function(mode) {
    path <- aic_workbook_paths[[mode]]
    
    if (!file.exists(path)) {
      stop("Selection workbook not found: ", path)
    }
    ...
    aic_tbl <- readxl::read_excel(path, sheet = "AIC_Table") %>%
      dplyr::mutate(
        ...
        degree_fs_patient = as.numeric(degree_fs_patient),
        degree_fs_curve = as.numeric(degree_fs_curve)
      )
    ...
    selected_tbl <- readxl::read_excel(path, sheet = "Selected_Parameters") %>%
      dplyr::mutate(
        ...
        degree_fs_patient_selection_status = as.character(degree_fs_patient_selection_status),
        degree_fs_curve_selection_status = as.character(degree_fs_curve_selection_status)
      )
    
    best_models <- aic_tbl %>%
      dplyr::group_by(file_name, analysis_mode) %>%
    ...
    best_models_with_k <- best_models %>%
      dplyr::left_join(
        selected_tbl %>% dplyr::rename(selected_model = model),
        by = c("file_name", "analysis_mode", "selected_model"),
      ...
      dplyr::mutate(
        ...
        selected_degree_fs_patient = dplyr::coalesce(degree_fs_patient_from_selected, degree_fs_patient_from_aic),
        selected_degree_fs_curve = dplyr::coalesce(degree_fs_curve_from_selected, degree_fs_curve_from_aic)
      ) %>%
      dplyr::select(
        file_name, analysis_mode, selected_model, selected_model_AIC,
        selected_degree_global, selected_degree_fs_patient, selected_degree_fs_curve,
        selection_status,
...
get_preselected_model_info <- function(file_name,
                                       analysis_mode,
  ...
  hit <- precomputed_selection[[analysis_mode]]$Best_Model %>%
    dplyr::filter(file_name == trimws(as.character(file_name))) %>%
    dplyr::slice(1)
  
  if (nrow(hit) == 0) {
    stop("No precomputed model-selection row found for file ", file_name, " in mode ", analysis_mode)
  }
  
  selected_model <- hit$selected_model[[1]]
  selected_degree_global <- hit$selected_degree_global[[1]]
  selected_degree_fs_patient <- hit$selected_degree_fs_patient[[1]]
  selected_degree_fs_curve <- hit$selected_degree_fs_curve[[1]]
  ...
  if (selected_model == "model1") {
    selected_degree_fs_patient <- NA_real_
    selected_degree_fs_curve <- NA_real_
  }
  
  if (selected_model == "model2") {
    if (is.na(selected_degree_fs_patient)) {
  ...
  if (selected_model == "model3") {
    ...
    if (is.na(selected_degree_fs_curve)) {
  ...
  list(
    ...
    degree_fs_patient_selection_status = hit$degree_fs_patient_selection_status[[1]],
    degree_fs_curve_selection_status = hit$degree_fs_curve_selection_status[[1]]
  )
}
...
# Output helpers
# ============================================================
smooth_table_from_bam <- function(model_obj, file_name, analysis_mode, selected_model_name) {
  ...
  out %>%
    dplyr::select(file_name, analysis_mode, selected_model, term, dplyr::everything())
}

param_table_from_bam <- function(model_obj, file_name, analysis_mode, selected_model_name) {
...
diagnostic_summary_from_bam <- function(model_obj, file_name, analysis_mode, selected_model_name) {
...
gam_vcomp_table <- function(model_obj, file_name, analysis_mode, selected_model_name) {
  ...
  out %>%
    dplyr::select(file_name, analysis_mode, selected_model, component, dplyr::everything())
}

k_check_table_from_bam <- function(model_obj, file_name, analysis_mode, selected_model_name) {
...
# ============================================================
# Fit selected model using externally selected degrees
# ============================================================
fit_selected_model <- function(df,
                               selected_model_name,
                               degree_global,
                               degree_fs_patient,
                               degree_fs_curve,
...
build_population_prediction_grid <- function(df, time_grid) {
  unit_df <- df %>%
    ...
    dplyr::arrange(Group, patient, repetition, curve)
  ...
  pred_grid_full <- tidyr::expand_grid(
    ...
    dplyr::select(time, Group, patient, repetition, curve)
  
  pred_grid_full$Group <- factor(pred_grid_full$Group, levels = levels(df$Group))
  pred_grid_full$patient <- factor(pred_grid_full$patient, levels = levels(df$patient))
  pred_grid_full$repetition <- factor(pred_grid_full$repetition, levels = levels(df$repetition))
...
recode_mode_data <- function(df, analysis_mode) {
  ...
  if (analysis_mode == "MN") {
    return(
      df %>%
        dplyr::filter(original_group %in% c("N", "M")) %>%
        dplyr::mutate(Group = factor(original_group, levels = c("N", "M")))
    )
  }
...
# ============================================================
# Pairwise mgcv helpers
# ============================================================
pair_to_analysis_mode <- function(group_1, group_2) {
  pair_sorted <- sort(c(as.character(group_1), as.character(group_2)))
  
  if (identical(pair_sorted, c("M", "S"))) {
    return("MS")
  }
  
  if (identical(pair_sorted, c("N", "S"))) {
    return("NS")
  }
  
  if (identical(pair_sorted, c("M", "N"))) {
    return("MN")
  }
...
pairwise_mgcv_smooth_tests <- function(df_file_original,
                                       file_name,
                                       precomputed_selection,
                                       rho_value,
                                       fallback_degree_global = 20,
                                       fallback_degree_fs_patient = 20,
                                       fallback_degree_fs_curve = 20) {
  available_groups <- sort(unique(as.character(stats::na.omit(df_file_original$original_group))))
  
  if (!all(c("N", "M", "S") %in% available_groups)) {
    return(data.frame())
  }
  ...
  out_list <- lapply(group_pairs, function(pair) {
    ...
    pair_model <- fit_selected_model(
      ...
      degree_fs_patient = selected_info_pair$degree_fs_patient,
      degree_fs_curve = selected_info_pair$degree_fs_curve,
      rho_value = rho_value
    )
    ...
    sm %>%
      ...
      dplyr::mutate(
        ...
        pair_degree_fs_patient = selected_info_pair$degree_fs_patient,
        pair_degree_fs_curve = selected_info_pair$degree_fs_curve,
        ...
        pair_degree_fs_patient_selection_status = selected_info_pair$degree_fs_patient_selection_status,
        pair_degree_fs_curve_selection_status = selected_info_pair$degree_fs_curve_selection_status,
        interpretation = "Pairwise BAM smooth-term test fitted with the pair-specific preselected model from the corresponding BAM_<mode>_all_files_model_comparison workbook"
      ) %>%
      dplyr::select(
        file_name, analysis_mode, selected_model,
        pairwise_mode, comparison, group_1, group_2,
        pair_selected_model_AIC,
        pair_degree_global, pair_degree_fs_patient, pair_degree_fs_curve,
        pair_selection_status,
        pair_degree_global_selection_status,
        pair_degree_fs_patient_selection_status,
        pair_degree_fs_curve_selection_status,
        interpretation,
        dplyr::everything()
      )
  })
...
build_significant_windows_from_pairwise <- function(pairwise_diffs) {
  ...
  sig_source <- pairwise_diffs %>%
    dplyr::arrange(file_name, analysis_mode, selected_model, prediction_type, comparison, time) %>%
    dplyr::group_by(file_name, analysis_mode, selected_model, prediction_type, comparison) %>%
    dplyr::mutate(
  ...
  sig_source %>%
    dplyr::group_by(file_name, analysis_mode, selected_model, prediction_type, comparison, block_id) %>%
    dplyr::summarise(
      start_time = min(time),
      end_time = max(time),
    ...
    dplyr::arrange(file_name, comparison, start_time)
}
...
build_all_groups_different_windows <- function(pairwise_diffs) {
  ...
  split_keys <- pairwise_diffs %>%
    dplyr::distinct(file_name, analysis_mode, selected_model, prediction_type)
  ...
  for (i in seq_len(nrow(split_keys))) {
    ...
    out_list[[length(out_list) + 1]] <- all_groups_source %>%
      ...
      dplyr::mutate(
        file_name = as.character(key$file_name),
...
# ============================================================
# Run one file + one mode
# ============================================================
run_one_analysis <- function(file_path,
                             analysis_mode,
                             precomputed_selection,
                             alpha = 0.05,
                             nsim_simultaneous = 10000,
  ...
  if (!analysis_mode %in% c("MS", "NP", "NS", "MN", "three_groups")) {
    stop("analysis_mode must be one of: 'MS', 'NP', 'NS', 'MN', 'three_groups'")
  }
  ...
  if (!overall_correction_requested %in% c("none", "holm", "bonferroni", "BH")) {
    stop("overall_correction_requested must be one of: 'none', 'holm', 'bonferroni', 'BH'")
  }
  ...
  if (dplyr::n_distinct(df$curve) < 2) stop("Fewer than 2 curves present after filtering.")
  ...
  degree_fs_patient <- selected_info$degree_fs_patient
  degree_fs_curve <- selected_info$degree_fs_curve
  
  selected_model <- fit_selected_model(
    df = df,
    selected_model_name = selected_model_name,
    degree_global = degree_global,
    degree_fs_patient = degree_fs_patient,
    degree_fs_curve = degree_fs_curve,
    rho_value = rho_value
  )
  
  aic_table <- precomputed_selection[[analysis_mode]]$AIC_Table %>%
    dplyr::filter(file_name == as.character(file_name)) %>%
    dplyr::mutate(
      analysis_mode = analysis_mode,
  ...
  overall_global_test <- selected_smooth_terms %>%
    ...
    dplyr::select(file_name, analysis_mode, selected_model, interpretation, term, dplyr::everything())
  ...
  pairwise_diffs <- dplyr::bind_rows(pairwise_list) %>%
    ...
    dplyr::select(file_name, analysis_mode, selected_model, prediction_type, dplyr::everything())
  ...
  if (inherits(concurvity_obj, "try-error")) {
    ...
    diagnostics_concurvity$file_name <- file_name
    ...
    diagnostics_concurvity <- diagnostics_concurvity %>%
      dplyr::select(file_name, analysis_mode, selected_model, measure, dplyr::everything())
  }
  ...
  run_info <- data.frame(
    ...
    degree_fs_patient_selection_status = selected_info$degree_fs_patient_selection_status,
    degree_fs_curve_selection_status = selected_info$degree_fs_curve_selection_status,
    alpha = alpha,
...
safe_run_one_analysis <- function(...) {
  tryCatch(
    {
      out <- run_one_analysis(...)
...
# ============================================================
build_mode_workbook <- function(files,
                                analysis_mode,
                                precomputed_selection,
                                output_folder,
                                alpha = 0.05,
  ...
  for (i in seq_along(files)) {
    file_path <- files[i]
    message("Running ", basename(file_path), " | mode = ", analysis_mode)
    
    res <- safe_run_one_analysis(
      file_path = file_path,
      analysis_mode = analysis_mode,
      precomputed_selection = precomputed_selection,
      alpha = alpha,
  ...
  # ------------------------------------------------------------
  # Workbook-level multiplicity correction across all files
  # For three_groups pairwise tables, this spans all files
  ...
  about_sheet <- data.frame(
    ...
    description = c(
      paste("Workbook for selected BAM inference by file. Mode:", analysis_mode),
      "Run settings and externally selected best model for each file; includes workbook-level corrected overall p-values across all files in this workbook.",
      "External AIC rows used for each file, annotated with the selected model.",
      "Overall test of whether group trajectories differ over time in the selected model. Workbook-level multiplicity correction is applied across all files in this workbook.",
      "Selected-model parametric terms after removing Group rows.",
      "Selected-model smooth terms.",
      "Population-averaged group curves obtained from full-model predictions averaged over observed patient/curve units within each group; includes CI and SD bands.",
      "One global test per pairwise contrast from simultaneous-band inference. Workbook-level multiplicity correction is applied within this workbook only. In three_groups, this spans all files and all three pairwise modes together within this workbook.",
      "Pairwise BAM smooth-term tests. For three_groups, each pair uses its own preselected model from the corresponding pairwise BAM_<mode>_all_files_model_comparison workbook. Workbook-level multiplicity correction is also applied within this workbook only.",
      ...
      "Time windows where all pairwise contrasts are simultaneously significant after workbook-level multiplicity correction; relevant for three_groups only.",
      ...
      "k.check() table for the selected model.",
      "Success/error log for file-wise processing."
    ),
    stringsAsFactors = FALSE
  )
  ...
  output_file <- file.path(
    output_folder,
    paste0("BAM_Inference_Preselected_", analysis_mode, "_selected_strains.xlsx")
  )
...
# ============================================================
# Load precomputed selection tables
# ============================================================
precomputed_selection <- load_precomputed_selection(aic_workbook_paths)
