...
class FiLMCNNEncoder(nn.Module):
    ...
    def forward(self, x_ts: torch.Tensor, x_tab: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.cat([x1, x2], dim=1)  # [B, out_channels, T]
...
class FusionCNN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        fusion_in = self.ts_encoder.out_dim + self.tab_encoder.out_dim
...
class FiLMCNN(nn.Module):
    """
    Tabular features condition the TS encoder via FiLM.
    No tabular embedding is concatenated at the classifier head.
    """
    def __init__(
        self,
        in_channels: int,
        ...
        spatial_resolution: int = 12,
        dropout: float = 0.3,
        tab_hidden: int = 64,      # reused as FiLM hidden size
        fusion_hidden: int = 64,   # classifier hidden size
        **kwargs,
    ):
        super().__init__()
...
def train_epoch_multimodal(
    ...
    *,
    ...
    for step, batch in enumerate(loader, start=1):
        Xts, Xtab, yb, _ = _unpack_batch(batch)
...
@torch.no_grad()
def eval_patient_outputs_multimodal(
    model: nn.Module,
    loader: DataLoader,
    device: torch.device,
    *,
    ...
    for batch in loader:
        Xts, Xtab, yb, pb = _unpack_batch(batch)
        if pb is None:
            raise ValueError("eval_patient_outputs_multimodal requires patient ids in the dataset.")
...
# -----------------------------
def train_fixed_epochs_multimodal(
    model,
    X_ts_tr,
    X_tab_tr,
    y_tr,
...
def suggest_params(trial: optuna.Trial, ModelClass: Any, *, max_train_epochs: int) -> Dict[str, Any]:
    ...
    if ModelClass in [FusionCNN, FusionTCN]:
        params["tab_out"] = trial.suggest_categorical("tab_out", [16, 32, 64])
    params["fusion_hidden"] = trial.suggest_categorical("fusion_hidden", [32, 64, 128])

    if ModelClass in [FusionTCN, FiLMTCN]:
...
def build_model_from_params(
    ...
    if ModelClass in [FusionCNN, FusionTCN]:
        kwargs["tab_out"] = int(params["tab_out"])
...
def inner_objective_nested(
    ...
    *,
    ...
    for fold_idx, (tr_pat_idx, va_pat_idx) in enumerate(skf.split(unique_pats, pat_labels)):
        ...
        X_ts_tr, X_tab_tr, y_tr, pat_tr = (
            X_ts_train[tr_mask],
            X_tab_train[tr_mask],
            y_train[tr_mask],
            patients_train[tr_mask],
        )
        ...
        model = train_fixed_epochs_multimodal(
            model,
            X_ts_tr,
            X_tab_tr,
            y_tr,
        ...
        _, y_true_pat, y_pred_pat, _ = eval_patient_outputs_multimodal(model, val_loader, device=device)
...
def run_nested_split(
    ...
    *,
    ...
    patient_ids, y_true_pat, y_pred_pat, proba_pat = eval_patient_outputs_multimodal(
